...
import { db } from "./db";
import { ForbiddenError, requireOwner, requireOwnedProject } from "./authz";
import {
...
// Auth gate: `requireOwner` / `requireOwnedProject` / `ForbiddenError` live in
// lib/authz.ts (a PLAIN module — helpers must not be exported from this
// "use server" file, or Next registers them as invocable server actions).
