...
| **Baseline_ObsidianDB** (phone) | Obsidian mobile, **phone-local only** | 136 files / 409 folders shown | Mirrors Baseline_Vault's structure but is a **diverged standalone copy** — verified NOT on the laptop (no such path on disk; laptop registers one vault; laptop Sync plugin enabled but **no sync.json** → no remote pairing exists). OD-A5 therefore includes a **merge-on-connect** step: wire both devices to one Sync remote so phone-side captures upload and reconcile (nothing orphaned), THEN set selective sync. The capture-app "Baseline Inbox" destination presumably routes into this phone vault — its captures are currently stranded there. |
| **THIS_ONE_VAULT** (phone) | Obsidian mobile, phone-local | contains a fresh `RONIN_DOJO-Baseline` duplicate | Created remotely by the operator 2026-07-18 (during this planning session) as a deliberate staging copy — a **known-fresh duplicate, NOT drift** (nothing to reconcile; the name signals canonical intent). OD-A treats it as a staging candidate; hold further duplication until Workstream A runs — every additional copy adds a reconciliation branch. |
