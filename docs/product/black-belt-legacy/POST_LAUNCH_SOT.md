...
| FI-023 | **Scrollytelling vertical timeline not on the profile page** (the USP; Tony Hua's favorite) | feature | P0 (pre-send gate) | **resolved (corrected + completed SESSION_0521)** — SESSION_0520's "ALREADY renders on `/directory/[slug]` AND `/me`" was **half wrong**: the ancestry scrollytelling (`LineageStorySequence` via `AncestrySection`) rendered on `/directory/[slug]` only; the `/me` owner arm never imported it (the 0520 screenshot conflated `/me`'s rank-history timeline with the ancestry story). SESSION_0521 (FI-024 H3) added the ancestry walk to `loadProfileViewForOwner` + rendered `AncestrySection` on `/me` — Doug-verified rendering with a real up-chain. Two timelines still exist: cohort "Cinematic explorer" = `/lineage/[treeSlug]` default; ancestry scrollytelling = the profile one (now genuinely both arms). Remaining prominence-on-consolidated-profile + explorer view toggle → WL-P2-37 next session + G-008 | FI-001 blocker — CLEARED |
| FI-024 | **Profile design-consistency pass** (following session): profile page aligned with ronin-dojo-monorepo BBLApp (parts parity); **edit = one inline-in-place surface** — button toggles edit on the profile page itself (user or admin) by porting the existing `LineageProfileDrawer` inline-edit pattern; **retire the separate profile edit pages** (one-surface law); finish killing URL photo fields → the one uploader family; belt cards readable/show-what's-filled; timeline scrollytelling design pass | design | P1 (pre-send gate) | **resolved (SESSION_0521)** — all 4 HIGH items shipped + Doug 9.6 launch-safe (H1 round-trip empirically persists). Two spec corrections: the drawer had NO inline-edit pattern (real port source = `PassportEditor` in the new `profile-edit-drawer.tsx`; operator-accepted), and "retire edit pages" = **editor-role only** (`/app/profile` 9-tab dashboard KEPT, operator). H3 correction: the scrollytelling was NOT on `/me` (contra FI-023's "present on both") — now rendered on the owner arm. Deferred: two-save collapse → WL-P2-45; video URL field → WL-P2-28 (excepted til A5). Timeline-on-consolidated-profile prominence continues in WL-P2-37 (next session). | FI-001 blocker — **CLEARED**; only the operator's "send Brian now" remains |
