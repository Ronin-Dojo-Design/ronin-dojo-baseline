...
| FI-025 | **Admin "Update User" does not persist** — clicking Update on the `/app` admin user surface saves nothing (or reverts on nav). Grounding: no `revalidatePath` in `server/admin/users/actions.ts` — smells like the `/admin`→`/app` stale-revalidate class | bug | P0 (pre-send gate) | open (SESSION_0520 operator) | FI-001 blocker |
| FI-021 | **Belt editing locked to White Belt 4-stripes** — operator can edit that one belt's date/promoter/school but cannot edit or add any other belt. Recently-touched area (0519 RankEntry writer consolidation) | bug | P0 (pre-send gate) | open (SESSION_0520 operator) | FI-001 blocker |
| FI-022 | **Certificates page is inert** — `/app/certificates` surface (table/new/[id]/delete) exists but "does nothing"; likely unwired actions rather than missing pages | bug | P1 (pre-send gate) | open (SESSION_0520 operator) | FI-001 blocker |
| FI-023 | **Scrollytelling vertical timeline not on the profile page** — the cohort/scrollytelling timeline (the USP; Tony Hua's favorite) is built + mounted in lineage View A but not surfaced visibly and not on the profile page. Belongs on Brian's own profile; top visual priority. Needs a design pass (folded into FI-024's design lane) | feature | P0 (pre-send gate) | open (SESSION_0520 operator) | FI-001 blocker |
