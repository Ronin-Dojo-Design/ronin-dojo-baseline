---
title: "Project Log Archive — Task review log (Hostile close review)"
slug: project-log-archive-task-review-log
type: archive
...
**Result:** `DATABASE_PUBLIC_URL` and `NEXT_PUBLIC_PLAUSIBLE_DOMAIN` are now optional env schema/example additions. `services/db.ts` uses `DATABASE_PUBLIC_URL` only during Next production build phase when set, falling back to `DATABASE_URL`; Prisma CLI migration routing remains in `prisma.config.ts` via a direct Neon URL and now defensively strips `-pooler` if a pooled Neon URL is accidentally supplied. The stale `GOOGLE_GENERATIVE_AI_API_KEY` env declaration was removed after source search found no runtime reads. `REDIS_REST_*` and `RESEND_AUDIENCE_ID` remain because current code still uses them. The new Vercel deploy runbook records active `apps/web` root settings and the env parity guard.
...
### SESSION_0209 — Dirstarter uplift L6 Base UI migration Phase 1 (foundation + leaf primitives)

| Task ID | Description | Status |
| --- | --- | --- |
| SESSION_0209_TASK_01 | Cody: install Base UI deps (`@base-ui/react ^1.3.0`, `cmdk-base ^1.0.0`, `tailwind-variants ^3.2.2`); port `apps/web/lib/slot.ts`; reconcile `apps/web/components/common/toaster.tsx` (next-themes + CSS-variable styling); relocate `components/web/empty-list.tsx` → `components/common/empty-list.tsx` and repath 10 import sites. | complete |
| SESSION_0209_TASK_02 | Cody: migrate `apps/web/components/common/separator.tsx` (Radix → `@base-ui/react/separator`; 0 `decorative` consumer sites) and `apps/web/components/common/avatar.tsx` (Radix → `@base-ui/react/avatar`; 11 consumer sites unchanged). | complete |
| SESSION_0209_TASK_03 | Doug + Petey: open drift register entry `D-016` (Radix→Base UI migration lane) with phased checklist (Phase 1 ticked; Phases 2-8 staged); update `.dirstarter-upstream` partial-port note; update epic L6 with status note + reference to D-016 + petey-plan-0083; append lane-ledger row; update project-log; verify typecheck/biome/test/build/wiki-lint/Vercel readiness; commit and push; full-close. | complete |

**Notes:** L6 mid-bow-in reconciliation (Petey) revealed that the epic's "reconciled easy wins" were ~95% already shipped in Ronin (skeleton composites, EmptyList, toast on all mutation forms, web Cmd-K via `search.tsx`, shared `DeleteDialog` with 18 per-entity wrappers, tooltips widely wired). Operator grilled and chose to escalate the substantive missing item (Radix→Base UI primitive runtime mismatch — 23 Ronin primitives on `radix-ui` vs 18 upstream primitives on `@base-ui/react`) to a dedicated multi-session lane rather than wedging it piecemeal. Phase 1 plants the foundation (deps + slot util + 2 safe leaf primitives + 2 drift reconciles) and opens drift register `D-016` to track Phases 2-8 (lib/utils cva→tv, Slot-only primitives, tooltip, accordion+hover-card, form primitives, popover family, command+tabs+admin Cmd+K+dep cleanup). Petey lane plan at [petey-plan-0083](../../sprints/petey-plan-0083.md).

**Result:** Ronin now has `@base-ui/react ^1.3.0`, `cmdk-base ^1.0.0`, and `tailwind-variants ^3.2.2` installed (latter two unused until later phases). `apps/web/lib/slot.ts` provides the Base UI `mergeProps`-based `slot()` util. `Toaster` adopts upstream's `next-themes`-aware shape and CSS-variable theming. `EmptyList` lives at its canonical `components/common/` path; the 10 existing consumers now import from there. `Separator` and `Avatar` are the first two leaf primitives migrated from `radix-ui` to `@base-ui/react` with no consumer-API breakage. `radix-ui` and `cmdk` remain installed because Phases 2-8 still depend on them; final removal happens in Phase 8 (SESSION_0216).

#### Review

##### SESSION_0209_REVIEW_01 — Hostile close review for L6 Base UI migration Phase 1

- **Reviewed tasks:** SESSION_0209_TASK_01, SESSION_0209_TASK_02, SESSION_0209_TASK_03.
- **Dirstarter docs check:** epic block `L6 — SESSION_0209 — UI primitives Part 2 (reconciled easy wins)` re-read; cross-referenced against upstream `7e724b6` `components/common/`, `lib/slot.ts`, `lib/utils.ts`, and `package.json`. The live `dirstarter.com` docs were not required — upstream source files are the contract for primitive migration.
- **Sources:** upstream `7e724b6` `components/common/{toaster,separator,avatar,tooltip,command,badge,box,button,card,heading,stack,form,animated-container,hover-card,accordion}.tsx` and `lib/{slot,utils}.ts` + `package.json`; Ronin existing `components/common/*.tsx`, `lib/utils.ts`, `app/layout.tsx` (ThemeProvider wiring), `package.json`, all `apps/web/**` consumer call-sites of `empty-list`, `asChild`, `<Tooltip tooltip=>`; Graphify query `SESSION_0209 L6 UI primitives skeleton tooltip command-k cmd-k toast sonner kbd empty-state` (budget 2500, 501 nodes); `pnpm --filter dirstarter typecheck`, `bun run lint` (biome --write), `bun test --isolate --path-ignore-patterns='e2e/**' --concurrency=1`, `pnpm --filter dirstarter build`, `bun run wiki:lint`.
- **Verdict:** Pass. No P0/P1 findings. Honest mid-session re-scoping (originally-planned "Slot-only primitives" turned out to also adopt upstream's `useRender` + `render={…}` consumer API replacing `asChild` — 44 call sites; original "leaf Radix primitives" included tooltip whose `<Tooltip tooltip=…>` wrapper has 41 call sites that need composition rewrites) was the right Petey call: shipped 2 truly-safe leaf migrations (separator, avatar) + foundation + reconciles, deferred the call-site-heavy migrations to phases 2-8 under `D-016`. Verification suite green at L5 baseline (244/244 tests, typecheck/biome/build clean). The parallel `--isolate` test segfault is a Bun 1.3.13 runtime crash that reproduces on a fresh checkout — not caused by our changes; sequential (`--concurrency=1`) runs pass clean.
- **Follow-up:** SESSION_0210 = Phase 2 of `D-016` (`lib/utils.ts` cva→tailwind-variants migration + Slot-only primitives with zero `asChild` call sites: `box.tsx`, `heading.tsx`, `animated-container.tsx`). Phases 3-8 staged in petey-plan-0083 per the D-016 checklist.

### SESSION_0210 — Dirstarter uplift L6 Base UI migration Phase 2a (utils + AnimatedContainer + cva import sweep)

| Task ID | Description | Status |
| --- | --- | --- |
| SESSION_0210_TASK_01 | Cody: rewrite `apps/web/lib/utils.ts` as tailwind-variants re-export (`tv as cva`, `cn as cx`, `VariantProps`; drop unused `compose`/`defineConfig`/`extendTailwindMerge` wiring; keep `popoverAnimationClasses` Radix-shape — Phase 7 swaps to Base UI semantics). Repath 2 stray `import { cx } from "cva"` consumers (`apps/web/components/web/ads/ads-picker.tsx`, `apps/web/components/admin/sidebar.tsx`) onto `~/lib/utils`. | complete |
| SESSION_0210_TASK_02 | Cody: migrate `apps/web/components/common/animated-container.tsx` from `radix-ui` `Slot.Root` to `slot()` from `~/lib/slot`; 12 consumer call sites unchanged. | complete |
| SESSION_0210_TASK_03 | Doug + Petey: update D-016 (tick Phase 2a, insert Phase 2b/2c rows, shift Phases 3-8 to SESSION_0213-0218); update petey-plan-0083 with re-phasing; append L6 Phase 2a row to lane-ledger; append `.dirstarter-upstream` partial-port note; update project-log + this review row; verify typecheck/biome/test/build/wiki-lint/Vercel readiness; commit and push; full-close. | complete |

**Notes:** Phase 2 was re-split at bow-in. Original framing assumed Box/Heading/AnimatedContainer were "mechanical Slot-only swaps." Bow-in audit (Petey) discovered upstream Box only exports `boxVariants` — the `Box` component is gone (59 `<Box>` JSX sites + 14 internal-primitive consumers in Ronin would need refactor) — and upstream Heading adopts `useRender` + `render={…}`, removing both legacy `as` and `asChild` props (140 `<Hn as="…">` rewrites in Ronin). Only AnimatedContainer is truly mechanical. Operator approved the re-split: 2a (this session) + 2b (Heading, SESSION_0211) + 2c (Box, SESSION_0212). Phases 3-8 slide to SESSION_0213-0218. `popoverAnimationClasses` content stays Radix-shape this phase — 4 still-Radix popover-family consumers (`popover.tsx`, `hover-card.tsx`, `dropdown-menu.tsx`, `select.tsx`) would break with Base UI semantics until Phase 7 migrates them. `cva` package stays installed through Phases 2-7; removal bundled with `radix-ui` + `cmdk` cleanup in Phase 8.

**Result:** `apps/web/lib/utils.ts` is now a thin tailwind-variants re-export. `AnimatedContainer` uses the Base UI `slot()` util from `~/lib/slot` with consumer API preserved (12 sites). Two stray `from "cva"` imports were consolidated onto `~/lib/utils`. D-016 + petey-plan-0083 reflect the re-phasing.

#### Review

##### SESSION_0210_REVIEW_01 — Hostile close review for L6 Base UI migration Phase 2a

- **Reviewed tasks:** SESSION_0210_TASK_01, SESSION_0210_TASK_02, SESSION_0210_TASK_03.
- **Dirstarter docs check:** upstream `7e724b6` `lib/utils.ts`, `components/common/{box,heading,animated-container}.tsx` directly compared against Ronin counterparts during bow-in audit. Live `dirstarter.com` docs not required.
