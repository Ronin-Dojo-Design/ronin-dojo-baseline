...
Surfaced by the **/techniques** sweep (public list + technique detail — decompose both routes + the filter bar; a fresh cloud session that had to bootstrap deps first):

- **A fresh cloud checkout fails `bun install` because the `postinstall` runs `prisma generate`, which needs `DATABASE_URL`.** The `@ronin-dojo/web` `postinstall` → `db:generate` reads `prisma.config.ts`, which throws `PrismaConfigEnvError: Cannot resolve environment variable: DATABASE_URL` and aborts the whole install — leaving NO `node_modules` and every later gate (`tsc`, `oxlint`, `next build`) unrunnable. Before the first `bun install` in a cloud session, export a throwaway `DATABASE_URL` (any well-formed Postgres URL — it is never connected to during generate) plus `SKIP_ENV_VALIDATION=1`; then `bun run db:generate` to materialize `~/.generated/prisma`. This is the §5a/§5b prerequisite that the DB-less sweeps still need: the Prisma *client types* must be generated for `tsc`/`next build` to typecheck, even when no DB is ever reached.
- **A bare `npx tsc --noEmit` reports false repo-wide errors in a never-built checkout — `next build`'s "Finished TypeScript" is the authoritative typecheck.** Next's typed-routes globals (`PageProps`, `LayoutProps`, `RouteContext`) live in `.next/types/**`, generated only by `next dev`/`next build`. In a clean checkout that has never built, `tsc` floods with `TS2304: Cannot find name 'PageProps'` across dozens of untouched route files (plus `react/jsx-runtime` noise if deps aren't installed yet). Don't chase these — they are not your code. Scope a quick check to your own files (`npx tsc --noEmit 2>&1 | grep <area>` → expect 0) for fast iteration, then run §5b `next build` for the real gate: it generates the route-type globals first, runs `✓ Compiled successfully`, and prints `Finished TypeScript in …s` (the full, authoritative typecheck) **before** the expected `P1001` at *Collecting page data*.
- **On a stacked sweep branch, `fallow audit --base main` mis-attributes every intervening sweep as "introduced" — pass the branch-point SHA.** `fallow audit` auto-detects/defaults its base to `main`, but the parallel sweep branches stack off a long-lived *integration* branch whose `main` can be dozens of commits behind. `--base main` then diffs in all those prior sweeps' files as your changeset (e.g. `complexity_introduced: 75`). Pass `--base <integration-HEAD-sha>` (the commit you branched from) so the attribution is *your* changeset only. Scoped correctly here: 17 changed files, `dead_code_introduced: 0`, `complexity_introduced: 1`, `duplication_introduced: 1`, `maintainability_avg: 92.5` (held), `avg_cyclomatic: 1.6`, `critical_complexity_pct: 0` — verdict `fail` purely on the raw above-threshold count, as the recipe warns.
- **A content-rich *detail* page's inline badge block relocates as one above-threshold component — same caveat as the events `AwardCard`.** The legacy `techniques/[slug]/page.tsx` rendered eight optional attribute badges (`category`/`position`/`difficultyLevel`/`discipline`/`isFoundational`/`isGi !== null`/`requiresPartner`/`requiresEquipment`) inline; decomposing lifts that branch-heavy block verbatim into `TechniqueBadges` (cyclomatic 10), so `complexity_introduced: 1` — even though the page's *own* complexity fell to a thin `params → fetch → notFound → <Orchestrator>`. It's a 1:1 relocation (the cyclomatic is inherent to the eight optional badges, not new), so enforce `dead_code_introduced: 0` and map the finding to its source, exactly like the events card. Do NOT atomize the eight-badge `Stack` into per-badge files to chase the count under threshold.
- **A surface whose colors were already token-clean makes /techniques a type-seam-only pass (cf. the /organizations sweep).** The technique cards/badges already render through the canonical `ListingCard` (ADR 0028) + semantic `Badge` variants with zero hex literals, and techniques carry no `Rank.colorHex` (so `BeltSwatch` does not apply). Step 2's *color* work was a verified no-op; the brand work was the *type* seam — wrapping each route body in `BrandTypography` with `bblHeadingScopeClass`. The list and detail routes are not BBL-font-wrapped at the layout level, so the scope is what carries the BBL seam where it belongs (and degrades to the app font off-BBL). Report it as already-color-correct rather than manufacturing color churn.

Surfaced by the **`/schools` + `/schools/[slug]` sweep** (the public school list + the 442-line detail monolith — another *data-heavy* server route built on the reused `ListingDetail` chrome, not `Intro`+`Section`):

- **`bblHeadingScopeClass` on the `BrandTypography` scope DOES reach a frozen shared component's H1 — without editing the component.** The `/me` sweep flagged that `ListingDetail` owns the page H1 (its `H2`→`h1`, baked `font-display`) and "a consumer cannot brand-font the title without editing the frozen component" because the component exposes no `titleClassName` slot. That's true for a *per-element* class you'd have to pass *into* the component — but it is NOT true for the **container** `bblHeadingScopeClass` (`[&_:is(h1,h2,h3,h4)]:…!`): it's a descendant selector on the wrapper, so it reaches the `ListingDetail` H1 (an `h1`) purely via CSS cascade, brand-fonting the title under BBL and degrading off-BBL — zero edits to the frozen component, still "the consumer authorises the tokens." So when a `ListingDetail`-based surface wants its H1 branded, wrap the body in `BrandTypography className={bblHeadingScopeClass}` rather than reaching for the (still-absent) `titleClassName`. The `/me` "flag, don't edit" note stands for a *per-element* override; the scope class is the clean route when the consumer is already wrapping in `BrandTypography`. Verified in the §5b emitted CSS — Lightning CSS merges the `:is(h1,h2,h3,h4)` rule with the existing `:is(h2,h3,h4)` one (`grep -roh 'is(h1,h2,h3,h4)…font-bbl-heading…!important' .next/static/chunks/*.css`).
- **`fallow audit`'s auto base-detection mis-attributes prior-session merged sweeps as `introduced` — pin `--base origin/main`.** Run bare `fallow audit` in a fresh worktree (where `bun install` regenerated `node_modules` + `.generated/prisma` and a `next build` left `.next/`) and it reported `changed_files_count: 272` with `dead_code_introduced: 14 / complexity 77 / duplication 55` — but inspecting the attributed files showed they were the **already-merged** course-detail / events / me-profile / organization-detail / lineage-canvas modules (NONE in this sweep's git diff). Its auto-detected base predated those merges, so it re-attributed them. The fix: scope to the real merge-base explicitly — `npx fallow audit --base origin/main --format json` collapsed it to `changed_files_count: 12` (exactly this sweep's files) and the truthful attribution: **`dead_code_introduced: 0`, `duplication_introduced: 0`, `complexity_introduced: 5`, `maintainability_avg: 89.1`** (held). Always pass `--base origin/main` (or `--changed-since`) in a worktree audit rather than trusting auto-detection, and read the *scoped* attribution.
- **A `ListingDetail` monolith decomposes around the same `*-data.ts` loader + thin orchestrator shape as an `Intro`/`Section` one — the chrome differs, the discipline doesn't.** `/schools/[slug]` (442 lines) split into `school-detail-data.ts` (server loader → `SchoolDetailView`; file-private `formatSchoolAddress`/`deriveInstructors`/`deriveClassesPerWeek`/`deriveSchoolInitials`, exporting only the loader + the view type), a thin `page.tsx` (`params → load → notFound() → <SchoolDetail {...view}/>`, keeping just `generateStaticParams`/`generateMetadata`), and one section file per `ListingDetail` slot (`school-hero-badges`/`school-intro`/`school-sidebar`/`school-about` eager; `school-instructors`/`school-programs`/`related-schools` `next/dynamic` below-fold). The 5 introduced complexity findings were all 1:1 relocations of the monolith's optional-field branch chains (`SchoolStructuredData` cyc 12 = the 7-spread JSON-LD; `SchoolSidebar` 9; `SchoolPrograms` 7; orchestrator 5 absorbing the render guards while `page.tsx`'s own complexity fell to a thin fetch) — same content-rich-card pattern as the events sweep. The `INSTRUCTOR_ROLE_CODES` set moved into the loader and is threaded down as `instructorRoleCodes` so the instructors section stays presentational.
- **A `Rank.colorHex` → `BeltSwatch` upgrade is OUT of scope when `colorHex` isn't already in the payload select.** The instructor cards render `m.rank.name` as a neutral text `Badge`; the school detail payload's `memberships.rank` select is `{ id, name, sortOrder }` — no `colorHex`. Even though `Rank.colorHex` exists in the schema (no migration needed), *adding it to the select* is a "new payload select" — the hard-boundary's `⛔ new payload selects that require schema` line. So unlike the events sweep (where `colorHex` was already on the wire and swapping the inline `rankStyle` ternary for `BeltSwatch` was a clean in-scope win), here the BeltSwatch upgrade is **flagged for the supervised lane**, and the rank stays the existing neutral text Badge (zero color literal either way). Rule: `BeltSwatch` adoption is in-scope only when `colorHex` is *already selected*; adding it to the select belongs to the supervised new-data lane.
