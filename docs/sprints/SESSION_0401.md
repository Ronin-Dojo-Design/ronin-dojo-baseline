...
# SESSION 0401 — Cinematic explorer mobile UX overhaul (badge removal · slim header · dropdown filter bar · canvas-overlay token pass)
...
#### SESSION_0401_TASK_01 — Design direction pass (Desi)
...
#### SESSION_0401_TASK_02 — Extract + unit-test the filter-match engine (Cody)
...
#### SESSION_0401_TASK_03 — Dropdown filter bar (Cody)
...
- **Done means:** flat pill row gone; dropdown bar filters the canvas (dim-not-hide preserved); static gates
  green.
- **Depends on:** SESSION_0401_TASK_01, SESSION_0401_TASK_02.

#### SESSION_0401_TASK_04 — Slim mobile header + remove badge + metric strip (Cody)
...
- **Depends on:** SESSION_0401_TASK_01.

#### SESSION_0401_TASK_05 — Canvas-overlay token + spacing + mobile pass (Cody)
...
#### SESSION_0401_TASK_06 — Gates + draft PR (Doug)
...
#### SESSION_0401_TASK_07 — Bow-out (Petey)
...
- **Depends on:** SESSION_0401_TASK_06.
...
| --- | --- | --- |
| SESSION_0401_TASK_01 | Desi | Design direction + primitive citations before any code. |
| SESSION_0401_TASK_02 | Cody | Pure engine extraction + unit tests (in-sandbox behavioural proof). |
| SESSION_0401_TASK_03 | Cody | Dropdown filter bar reusing L1 primitives. |
| SESSION_0401_TASK_04 | Cody | Slim header + badge removal. |
| SESSION_0401_TASK_05 | Cody | Canvas-overlay token/spacing pass. |
| SESSION_0401_TASK_06 | Doug | Gates + PR. |
| SESSION_0401_TASK_07 | Petey | Bow-out / closing ritual. |
...
| ID | Status | Summary |
| --- | --- | --- |
| SESSION_0401_TASK_01 | landed | Desi design-direction pass (see Review log). Filter primitive corrected at preflight: `DropdownMenu` (base-ui Menu, `closeOnClick=false` checkbox items) not `Popover` — checkbox items need Menu context. |
| SESSION_0401_TASK_02 | landed | `lib/lineage/filter-facets.ts` — pure `deriveFacets`/`matchMemberIds`/`nodeMatchesFacet`/`facetKey` with AND-across/OR-within; `filter-facets.test.ts` (11 cases). |
| SESSION_0401_TASK_03 | landed | Flat pill row → four `DropdownMenu` + `DropdownMenuCheckboxItem` dropdowns (Group/Belt/School/Year), count-badge triggers, belt swatches, per-dimension + global "Clear all". `Stack` layout, 2×2 on mobile. |
