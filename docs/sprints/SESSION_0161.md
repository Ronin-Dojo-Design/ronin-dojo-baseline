...
| SESSION_0161_TASK_07 | Fix Vercel production environment/config layer: add required Production env vars and override Framework Preset from `Other` to `nextjs` in `vercel.json`. | in progress |
...
- **TASK_07** — Added missing Vercel Production env vars via CLI: `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL`, `NEXT_PUBLIC_SITE_URL`, `NEXT_PUBLIC_SITE_EMAIL`. Vercel project inspect showed Framework Preset `Other` and Output Directory `public`; repo config now overrides framework to `nextjs` and builds only the `dirstarter` app.
...
- `vercel project inspect ronin-dojo-baseline --scope brian-scotts-projects-4841d4d6` confirmed:
  - Root Directory `.`
...
- Awaiting push/redeploy after TASK_07 framework config patch so Vercel can create a successful production deployment for the configured domain.
