...
- Follow-up E2E failure on `891ddb0`: Playwright exposed stale public-lineage drawer expectations after SESSION_0347 tier-gating made free listings compact/path-only (`canOpenProfileDrawer=false`). Remediation: anonymous public tests now assert free compact/no-drawer behavior, drawer rank-redaction runs through a seeded premium viewer entitlement, the post-claim public preview gets a premium entitlement fixture, and the DSR spec is explicitly serial to match its stateful auth setup.
- Follow-up E2E failure on `7da5ed1`: Chromium proved that serial DSR tests still receive fresh Playwright contexts, so the third DSR test was ordered correctly but unauthenticated. Remediation: the confirm-checkbox validation test now creates a fresh authenticated session for the existing seeded user and asserts the DSR row count stays unchanged.
