...
- **Steps:**
  1. Author `docs/architecture/decisions/0042-rank-award-fact-vs-member-milestone.md`.
...
| SESSION_0479_TASK_02 | landed | Added ADR 0042, updated ADR 0016/0035 links, lineage hub, schema wiki annotation, and ubiquitous-language entries |
...
- ADR 0042 records the fact/enrichment split; ADR 0016 and ADR 0035 now cross-reference it.
...
| `docs/architecture/decisions/0042-rank-award-fact-vs-member-milestone.md` | New ADR for `RankAward` fact vs `RankMilestone` enrichment |
| `docs/architecture/decisions/0016-lineage-promotion-source-of-truth.md` | Added SESSION_0479 amendment and backlink to ADR 0042 |
| `docs/architecture/decisions/0035-lineage-rank-display-from-awarded-truth.md` | Linked ADR 0042 as preserving awarded-truth display |
| `docs/architecture/ubiquitous-language.md` | Added `Belt Journey` and `RankMilestone`; refreshed `RankAward` wording |
| `docs/runbooks/domain-features/lineage-hub.md` | Added ADR 0042 and `RankMilestone` to the lineage schema map |
| `docs/knowledge/wiki/files/schema-prisma.md` | Refreshed schema counts and rank-model summary |
| `docs/knowledge/wiki/index.md` | Added ADR 0042 and SESSION_0479 rows |
...
- **Giddy:** Pass, 9.5/10. The slice followed the locked epic, kept schema additive, avoided privacy/verification
  ...
  `RankMilestone` as rank authority; ADR 0042 and the session handoff make that explicit.
...
- ADR update required and completed: `docs/architecture/decisions/0042-rank-award-fact-vs-member-milestone.md`.
...
| Step | Proof |
| --- | --- |
| JETTY/frontmatter sweep | Updated/stamped touched docs with `updated: 2026-07-01` and `last_agent: codex-session-0479` where applicable: ADR 0016, ADR 0035, ADR 0042, ubiquitous language, lineage hub, schema wiki, wiki index, SESSION_0479. |
| Backlinks/index sweep | Added ADR 0042 and SESSION_0479 to `docs/knowledge/wiki/index.md`; added reciprocal links among ADR 0016, ADR 0035, ADR 0042, ubiquitous language, lineage hub, and schema wiki. Wiki log checked and left superseded; component inventory checked and left unchanged. |
...
| Next session unblock check | Unblocked: Slice 3 can start from ADR 0042, `RankMilestone`, and the existing `RankAward` invariant docs. |
