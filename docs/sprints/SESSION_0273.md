...
- Patched public lineage route rendering after production deploy surfaced `DYNAMIC_SERVER_USAGE`: `/lineage` and `/lineage/[treeSlug]` now force dynamic rendering because brand resolution is request/host-derived, and the detail route no longer precomputes static params.
- Added `/admin/lineage`:
...
| `apps/web/app/(web)/lineage/page.tsx` | Forced dynamic rendering for request-brand lineage index reads. |
| `apps/web/app/(web)/lineage/[treeSlug]/page.tsx` | Added public claim action when tree is claimable; forced dynamic rendering and removed static params for request-brand lineage detail reads. |
...
| `pnpm --filter @ronin-dojo/web build` | Passed after removing ignored empty `apps/web/.env.production.local` generated by failed Vercel env pull; `/lineage` and `/lineage/[treeSlug]` now build as dynamic routes. Existing Turbopack/NFT warning remains in storage monitoring import trace. |
