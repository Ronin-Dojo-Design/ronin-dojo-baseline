...
| RH-2 | **Doc-root sprawl** — 11 `petey-plan-*.md` + `consolidation-merge-prompt.md` + `prune-roadmap.md` loose at `docs/`. | DONE (SESSION_0711 cleanup): 11 spent plans + `consolidation-merge-prompt.md` moved → `docs/_archive/petey-plans/` with inbound refs relinked; the 12 refuted (still-live) plans stay at docs root. | low | low (docs) |
