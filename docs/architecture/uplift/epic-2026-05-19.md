...
| --- | --- | --- | --- |
| SESSION_0204 | L1 — Baseline map refresh + lane-ledger init + env/deploy diff report (doc-only) | Doc | — |
| SESSION_0205 | L2 — Env/deploy implementation | Code (env/runtime) | L1 |
| SESSION_0206 | L3 — Schema port wave (aggressive) | Prisma/migration | L1 |
| SESSION_0207 | L4 — Baseline listings relabel + tier flow | Code (feature) | L3 |
...
- **Agent:** Cody
- **What:** Build the admin tier-transition panel per wireframe. Wire into `server/web/tools/actions.ts` with audit-log entries for every transition.
