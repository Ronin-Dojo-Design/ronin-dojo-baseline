...
## Current status (SESSION_0356 reconciliation — read this first)

The epic is **partially complete, not done.** Honest status as of 2026-06-07:

- **Done + ledgered (lane-ledger.md):** L1 env/deploy, L2–L3 schema wave, L4 listings/tier flow,
  and **L5–L6 the full Radix→Base UI migration (8 phases, drift D-016 CLOSED)** — the bulk of the work.
- **NOT executed:** L7 (vendor SDK polish), L8 (content/SEO), L9 (admin routing), **L10–L13 (oRPC ADR
  + 3 pilots)**, L14 (toolchain bump), L15 (final reconciliation). No lane-ledger rows exist for these.
- **oRPC specifically:** `@orpc/*` + `@tanstack/react-query` are **absent**; there is **no `/api/rpc`**;
  the "ADR_0019" the Locks table references was **never written** (that number was reused for
  membership-lifecycle). The decision is now recorded in **ADR 0024**
  (`decisions/0024-orpc-vs-next-safe-action.md`, status `proposed`) and the contradiction in **drift D-021**.
- **Toolchain:** L14 is effectively in place outside this ledger — the repo runs **Next 16.0.9 /
  React 19.2.2 / TS 5.9.3 / Base UI 1.3** today, so the bump does **not** block oRPC.
- **`.dirstarter-upstream` marker** is still `pending` (never bumped to `7e724b6`) → the epic is formally **open**.

The Lane index table below shows the *original plan*; treat the status above as the source of truth.
...
1. Read the specific lane block below for its session.
1. Run `graphify query` for the lane's nouns.
1. Confirm `vercel ls` shows the most recent prod/preview = `Ready` before running TASK_01.

Per-session bow-out must (per `docs/rituals/closing.md` + this epic's per-lane gate):

1. Doug hostile-close review.
1. Lane-ledger row appended with `lane`, `upstream_commits_ported`, `ronin_commit`, `dirstarter_upstream_marker_before`, `dirstarter_upstream_marker_after`, `verification_proof`.
1. `apps/web/.dirstarter-upstream` partial-port note bumped (full SHA bump to `7e724b6` only happens in SESSION_0218).
1. `vercel ls` Ready check (FS-0023 lesson).
1. Graphify refresh.
1. Commit + push `main`.
