...
1. `/lineage/join` confirmation and admin emails contained Baseline URLs even when submitted through `bbl.local` / `blackbeltlegacy.com`.
...
1. **Server actions resolve request origin.** `lib/brand-context.ts` exports `getRequestOrigin()` which reads `x-forwarded-host` / `host` headers to derive the origin URL. Server actions (e.g., `createJoinLegacyInterest`) use this for checkout links and admin notification links instead of `NEXT_PUBLIC_SITE_URL`.

1. **Production guard for missing senders.** If a brand-specific sender env var is not configured in production, the email helper throws a clear error rather than silently falling back to the Baseline sender (which would fail Resend domain verification).

1. **Dev origins.** `next.config.ts` includes `allowedDevOrigins` for `bbl.local`, `baseline.local`, `wekaf.local` so HMR works across brand hosts.
