...
1. **BBL** — port BBL landing + app screens; build the migration script ([ADR 0007](decisions/0007-bbl-migration.md)) in parallel.
1. **Baseline Martial Arts** — port TuffBuffs UI minus brand-specific copy/colors; rebrand visually.
1. **WEKAF** — greenfield rework (no users, no migration constraint).
...
1. If it uses state, hooks, browser APIs → add `'use client'` at the top.
1. Replace any of:
...
1. If it imports from `src/services/*`, that whole call chain dies — rewrite the data fetch as an `await` in a Server Component or `useQuery` (TanStack) in a client component, hitting `packages/api-client/`.
1. Verify in `bun dev` then commit per component.
