...
| `BeltSwatch` | `apps/web/components/common/belt-swatch.tsx` | `colorHex?`, `className?` | SESSION_0355: small belt-color swatch driven by `Rank.colorHex` data via a data-bound SVG `fill` (**no hardcoded colour, no inline `style`** — the brand-safe + no-inline-style rules). `aria-hidden` decorative; neutral muted-dot fallback when no `colorHex`. First consumer: the rank-award `DataSelect` rows. Reusable consolidation of the per-surface bespoke belt-color treatments. |
