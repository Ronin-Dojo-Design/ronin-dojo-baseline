...
| WL-P1-7 | `apps/web/components/common/select.tsx`; ~17 id-valued consumers | Bug (Base UI label) | Base UI `Select` renders the raw `value` (a cuid/slug) in the trigger when a value is preset from DB/URL and the popup hasn't mounted — because the item labels aren't registered until open. Affects every id-valued Select (rank "select for display", org/user/tier/technique/discipline/mat/fight/schedule/program/content selects). Surfaced SESSION_0353 from an operator report of rank dropdowns showing cuids. | ⏳ Partial — SESSION_0353 fixed the two highest-value consumers (`directory-filters.tsx` discipline, `lineage-selected-rank-select.tsx`) by passing `items` to `Select.Root`. Systemic fix open: pass `items` to each remaining `Select.Root` (or add a `DataSelect` wrapper taking `options:{value,label}[]`) + a render test asserting preset shows label not id. Full enumerated list in SESSION_0353_FINDING_01. |
