...
| WL-P2-16 | `apps/web/server/web/lead/public-actions.ts` (writes `lead.meta` refs); `apps/web/app/app/tools/*`, `apps/web/app/app/leads/[id]/page.tsx` (review surfaces) | Feature gap (render not wired) — SESSION_0441 | The creatable-combobox **registered** refs (`currentRankId`/`schoolOrgId`/`trainedUnderNodeId`/`representTreeId`) persist to `lead.meta` for the FREE/guest/non-claim intake — which creates a **Lead + Pending Tool, NOT a `PassportClaimRequest`** — so the Slice A "Lineage selections" card (claim-path only) never shows them. `findLeadById` already selects `meta`, but the lead detail + the Tool review render none of it. The steward sees the text in the claimant note / Tool `submitterNote`, but not the resolved link. | **Next (SESSION_0442 first build task — Slice B):** render the refs as resolved links on `/app/tools` (Pending Tool review) + `/app/leads/[id]`; resolve names server-side (registered = link, custom = text); mirror `app/app/lineage/claims/[id]/_components/claim-review-detail.tsx`'s Lineage selections card. |
