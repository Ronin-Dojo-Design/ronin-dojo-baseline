...
| WL-P2-17 | `apps/web/server/admin/*/queries.ts` (~24 files; e.g. `age-groups`, `categories`, `entitlements`, `users/queries.ts:8-33`) | Duplication (handroll slip) | fallow `dup:16999900` (31-line block × 24 instances) + `dup:c3bcb118` (26-line × 12) — the admin list/query `query-builder` scaffold (where / orderBy / paginate) is copy-pasted across ~24 `server/admin/*/queries.ts` files. **Pre-existing; surfaced (not introduced) by SESSION_0452 TASK_01** fallow-fix-loop on the 0451 revalidate diff (the swap merely touched `users/queries.ts`). | Extract one shared admin-query-builder helper (model + where + orderBy + pagination) and migrate the ~24 call-sites; verify per-file behavior unchanged. Dedicated refactor session — broad but mechanical; judge the result vs the `code-quality-matrix`. |
| WL-P2-18 | `server/admin/tournaments/actions.ts` (`upsertDivision:166`, `scoreMatch:849`, `seedable:681`, dead export `updateTournamentStatus:92`); `server/web/media/actions.ts:17 revalidateForTarget`; `server/admin/users/queries.ts:154 AddPersonOptions` (dead type export) | Fallow cleanup (complexity + dead exports) | fallow CRAP/complexity hotspots (`upsertDivision` CRAP 30, `scoreMatch` / `seedable` HIGH, `revalidateForTarget` CRAP 42) + 2 unused exports. **All pre-existing in files the 0451 revalidate swap touched; surfaced (not introduced) by SESSION_0452 TASK_01.** | Extract the oversized branches; remove or consume the unused exports (confirm no dynamic refs first). Lower priority than WL-P2-17; bundle into a tournament-admin or fallow-cleanup session. |
| WL-P2-19 | `apps/web/server/web/entitlements/queries.ts:40` (`canUploadMedia`); `lib/safe-actions.ts:101` (`mediaUploadActionClient`); `app/app/users/_components/user-actions.tsx:37` | RBAC gap (capability gate + discoverability) | **Operator-reported (SESSION_0452):** platform admins (Brian, Tony Hua) can't upload media from in-editor surfaces — `canUploadMedia` ORs S3_UPLOAD-entitlement / org-role / org-ownership but **never `User.role`**, so admins without those signals fail `mediaUploadActionClient` (`uploadMedia`/`fetchMedia`); the admin media *library* works (it's `adminActionClient`). Also the user role editor hardcodes `["admin","user"]` — `tournament_director` not selectable though the enum + `updateUserRole` support it. Per-user grant CRUD ALREADY exists (audited `grantUserEntitlement` + `UploadGrantToggle` on `/app/users/[id]`) → the gap is discoverability + role-honoring gates, **not** a missing system. ⚠ watch: `uploadMedia` passes a caller-controlled `path` to `uploadToS3Storage` (could target `certificates/`/`claim-evidence/` — ties to risk #6). Petey+Giddy research-review in SESSION_0452. | **Recommended (Petey+Giddy):** (1) unblock now — grant the seeded `S3_UPLOAD` entitlement to the 2 admins via the existing audited UI (zero code, reversible); (2) durable — route `canUploadMedia` through `can(user,"media.manage")` (admin already `["*"]`) so future admins auto-covered (mind the 60s `user-entitlements-*` cache tag on role change); (3) add `tournament_director` to the role editor; (4) consolidate a per-user capabilities panel only if discoverability needs it. **Do NOT** add a new per-user permission table (5th authz system). PR route (authz). |
| WL-P2-20 | `apps/web/server/admin/users/actions.ts:15` (`updateUser`), `:153` (`updateUserRole`) | Audit gap + self-escalation (security) | **SESSION_0452 RBAC review (Giddy):** both write `User.role` with **no `AuditLog`** and **no self-escalation guard** — any platform admin can silently promote anyone (incl. themselves) to `admin` (a risk #11 cross-org super-user). `deleteUsers` self-protects (`role:{not:"admin"}`) but the role-grant path doesn't. Directly unmet mitigation for security-register **#11** ("alert on unexpected `role:admin` grants"). Contrast: the entitlement-grant path (`grantUserEntitlement`→`admin-grants.ts`) IS audited (WL-P1-6) — the model to copy. | Audit role changes before mutation (clone the `admin-grants.ts` `AuditLog`-in-tx before/after + grantorUserId pattern); block self-role-elevation; consider alerting on a new `role:admin`. Promote to a risk-register row at the next security pass. PR route (authz). |
