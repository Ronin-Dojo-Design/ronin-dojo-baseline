...
| [SESSION_0411](../../sprints/SESSION_0411.md) | session--implement | closed — **Threaded BBL font variables into the lineage profile drawer portal.** Added a brand-neutral `contentClassName` seam on `LineageProfileDrawer`, merged it into the portaled `DrawerContent` popup without replacing desktop panel layout classes, and passed `cx(bblHeadingFont.variable, bblBodyFont.variable)` from both lineage drawer consumers (`LineageViewAIsland` and `LineageTreeBoard`). Kept `components/common/drawer.tsx` untouched/brand-neutral, added no Tailwind `font-bbl-heading` utility, and made no schema/data changes. Verification: format/typecheck/wiki-lint passed; oxlint had pre-existing warnings only; Docker/live-DOM was unavailable in the cloud container, so computed-style proof is deferred to PR review per the launch-sweep fallback. |
