...
| [SESSION_0399](../../sprints/SESSION_0399.md) | session--implement | closed — **Passport media parity on `/me`.** Added the `MediaAttachmentManager` to the canonical Passport editor for entry-point parity across identity surfaces (PR #72). Backfilled into the index at SESSION_0401 (FS-0019). |
| [SESSION_0400](../../sprints/SESSION_0400.md) | session--implement | closed — **D-023 shared field primitives + BBL-DISCOVER rank filter.** Extracted `TextField`/`TextAreaField`/`DateField`/`AvatarField` and folded both identity editors (lineage-node form + `PassportEditor`) onto them; wired the previously-dead `rank` URL param end-to-end (filter-options → People-only dropdown narrowed by discipline → `rankAwardsEarned` where-clause → facets) with where-builder unit tests. No schema change. Gates green; fallow exit 0. PR #73. Backfilled at SESSION_0401 (FS-0019). |
| [SESSION_0401](../../sprints/SESSION_0401.md) | session--open | closed — **Cinematic explorer mobile UX overhaul.** Removed the "Black Belt Legacy Explorer" eyebrow badge; slimmed the mobile header (compact title + thin metric strip, desktop unchanged); replaced the flat filter-pill row with four Apple-style multi-select dropdowns (Group/Belt/School/Year) composing the L1 `DropdownMenu` primitives; extracted the filter-match engine to a pure unit-tested helper (`lib/lineage/filter-facets.ts`, AND-across/OR-within). Fixed the mobile **width** overflow — the focal card was a fixed `w-72` cut off the viewport, now `w-[min(18rem,calc(100vw-4.5rem))] sm:w-72`. Petey→Desi→Cody→Doug. Gates: test 11/0, typecheck 0, lint/format/wiki/fallow exit 0. PR #75. |
