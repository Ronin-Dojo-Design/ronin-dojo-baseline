...
| [SESSION_0403](../../sprints/SESSION_0403.md) | session--implement | in-progress — **BBL go-live data: lineage pricing seed + Dirty Dozen profile import.** `scripts/seed-bbl-lineage-pricing.ts` seeds two paid tiers (Premium/Elite × monthly/annual, amounts from the monorepo BBLApp, `metadata.surface=lineage_membership`, cumulative entitlement grants, real BBL-account `stripePriceId`s from env) + `BBL_STRIPE_PRODUCTS_SPEC.md`; `scripts/import-bbl-lineage-profiles.ts` imports the 7 featured BBLApp black belts as claimable placeholder Passports + DirectoryProfiles, tagged into a "Dirty Dozen" `LineageVisualGroup` (Dirty Dozen comp = lifetime `LINEAGE_ELITE`). Follow-up to PR #76 (ADR 0030). No schema change. |
