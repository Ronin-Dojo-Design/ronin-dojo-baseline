...
| [SESSION_0391](../../sprints/SESSION_0391.md) | session--open | closed — **Phase 3b data migration rehearsed: placeholder Users removed, BBL people preserved as accountless Passports.** Added `RankAward.awardedByPassportId` for historical promoter identity, shared satellite metadata, `phase3b-user-carry-data.ts`, and guarded deferred drop SQL. Local rehearsal: 32 Users/23 placeholders/6 Passports → 9 Users/0 placeholders/30 Passports/23 accountless; preflight PASS. Safety finding: null old placeholder satellite `userId`s before hard-delete or cascades delete identity rows. Old-column drop/browser proof deferred to Phase 3c read-path repoint. |
