...
| 3 | Admin route reliance risk — **RESOLVED pending SESSION_0549 merge** | High → Low → N/A | SESSION_0549 retired the remaining app-code reliance: `config/admin-sections.ts` was already canonical `/app`; the dead `/admin` route shell + stale sidebar/command-palette stack were deleted; live route literals were repointed (`/admin/tools/*` → `/app/tools/*`, `/admin/leads/*` → `/app/leads/*`, `/admin/claims` → `/app/claims`, `/admin` → `/app`); `withAdminAuth` now gates through `can(..., content.manage)` instead of raw `role !== "admin"`; `rg 'revalidatePath(.*\\/admin' apps/web/server apps/web/app` is clean. Redirect shadowing check corrected `/admin/posts` from dead `/app/posts` to live `/app/blog`, and added legacy bookmark coverage for `/admin/task-board` → `/app/loop-board` plus `/admin/techniques` → `/app/techniques`. | At merge, keep only the intentional legacy redirect table/tests for `/admin/*` bookmarks; future app links must target `/app` directly. | Admin/auth |
...
| 13 | Prod Neon DB credential exposed in session transcripts — **rotation overdue** (SESSION_0449/0450) | High | The prod Neon role password leaked into local shell/transcript output **twice** (`psql` error messages echoed the conninfo; root-caused in `docs/_archive/project-log/03-task-review-log.md:992`). Credential is **still live (never rotated)**; exposure is bounded to local Claude transcripts. Confirmed by the SESSION_0452 sweep that the secret is **not** committed anywhere (lives only in gitignored `apps/web/.env.prod` + Vercel env). | Rotate the Neon role password using [`neon-credential-rotation.md`](../runbooks/database/neon-credential-rotation.md): Neon dashboard → update `apps/web/.env.prod` + Vercel `DATABASE_URL`/`DIRECT_URL` → redeploy + Prisma connectivity check; going forward pass creds via `PG*` env vars and redact `postgres://…` in any error output | Platform/devops |
...
| 16 | Preview builds hold PROD DB write-creds (SESSION_0730) | **✅ RESOLVED (SESSION_0738 / #398)** | The Vercel **preview** environment carried the prod Neon `DATABASE_URL`/`DIRECT_URL`. Consequence 1 (**MITIGATED** SESSION_0730): `prebuild` ran `migrate deploy` on every build, so a migration file in ANY pushed PR applied to prod at first preview build, pre-merge (proven: #397's migration `finished_at` predated its merge by ~2h; survivable only because additive — the queued #380 table-drop would have executed in prod at PR-open). Gated by `apps/web/scripts/prebuild-migrate.ts` (`VERCEL_ENV`-aware: production/local apply, preview SKIP). Consequence 2 (**RESOLVED** SESSION_0738): preview no longer reaches prod — prod `DATABASE_URL`/`DIRECT_URL` scoped **Production-only**; Preview points at a dedicated Neon `preview` branch; Standard Deployment Protection; `production` branch protected. | **✅ DONE (SESSION_0738):** env scoped Production-only; Preview→Neon `preview` branch; explicit preview-migration mechanism ratified + documented (`schema-migration.md`); Deployment Protection recorded. **Proof** (throwaway PR #408, reverted): Preview build logged `[prebuild-migrate] SKIP: VERCEL_ENV=preview`, prod `_prisma_migrations` gained 0 rows. **#398 CLOSED**; #380's stated blocker cleared (stays a separate gated lane). See D-058. | Platform/devops |
...
- **Rotate now:** reset the Neon role password in the dashboard; update
  ...
  production env vars; follow [`neon-credential-rotation.md`](../runbooks/database/neon-credential-rotation.md)
  and verify deploy + Prisma connectivity without printing connection strings.
