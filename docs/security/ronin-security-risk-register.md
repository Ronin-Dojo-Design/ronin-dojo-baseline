...
| 16 | Preview builds hold PROD DB write-creds (SESSION_0730) | **High** (migration firing path mitigated; runtime prod-credential exposure OPEN) | The Vercel **preview** environment carries the prod Neon `DATABASE_URL`/`DIRECT_URL`. Consequence 1 (**MITIGATED** SESSION_0730): `prebuild` ran `migrate deploy` on every build, so a migration file in ANY pushed PR applied to prod at first preview build, pre-merge (proven: #397's migration `finished_at` predated its merge by ~2h; survivable only because additive — the queued #380 table-drop would have executed in prod at PR-open). Now gated by `apps/web/scripts/prebuild-migrate.ts` (`VERCEL_ENV`-aware: production/local apply, preview SKIP). Consequence 2 (**OPEN**): preview *runtime* still reads/writes prod data with prod creds — every PR's preview URL is a prod-data surface. | **Operator (Vercel/Neon dashboards):** scope prod `DATABASE_URL`/`DIRECT_URL` to Production only; point Preview at a Neon branch; choose an explicit reviewed preview-migration mechanism because `prebuild-migrate.ts` intentionally skips preview. Prove with a throwaway additive PR: no prod migration row, preview branch migrated, render succeeds, DB identity logged. Record the Deployment Protection decision. #380 stays blocked until this lands. See D-058. | Platform/devops |
