...
- **Session:** SESSION_0641 (AM merge sweep + Mammoth client demo).
- **What happened:** `apps/rdd/vercel.json`'s `ignoreCommand` was `git diff --quiet HEAD^ HEAD -- apps/rdd packages bun.lock package.json ':(exclude)*.md' ':(exclude)*.mdx'`. On Vercel it evaluated to exit 0 (**"no change"**) even for commits that clearly touched `apps/rdd` (#264 `vercel.json`, #306 `/clients`), so every RDD Production build was auto-**Canceled** at the "Ignored Build Step" (5s, status Canceled). #306's `/clients` preview area was on `main` with its env vars set, yet the site 404'd because the deploy never ran. The **same command returned exit 1 (build) locally** — the failure only reproduced in Vercel's checkout (shallow-clone `HEAD^` resolution). Confirmed from the build log: `Running "git diff --quiet HEAD^ HEAD ..."` → `The Deployment has been canceled as a result of running the command defined in the "Ignored Build Step" setting.`
- **Root cause:** `HEAD^ HEAD` is fragile in Vercel's git checkout, and the gate defaulted to **skip** on ambiguity — the worst failure mode: a silent non-deploy that reads as a successful, up-to-date site.
