...
async function main() {
  ...
  const erik = await db.directoryProfile.findUnique({
    ...
    select: { id: true, slug: true, passport: { select: { displayName: true, lineageNode: { select: { slug: true } } } } },
  })
  if (!erik) throw new Error(`GUARD: Erik profile ${ERIK_PROFILE_ID} not found`)
  if (erik.slug !== ERIK_OLD_SLUG) {
    console.log(`FIX 1 (Erik slug): SKIP — slug is already "${erik.slug}", not "${ERIK_OLD_SLUG}" (already fixed?)`)
  } else {
    if (erik.passport?.lineageNode?.slug !== ERIK_NEW_SLUG) {
      throw new Error(`GUARD: Erik profile passport node is "${erik.passport?.lineageNode?.slug}", expected "${ERIK_NEW_SLUG}" — aborting`)
    }
    ...
    console.log(`FIX 1 (Erik slug): RENAME profile ${erik.id}  "${ERIK_OLD_SLUG}" → "${ERIK_NEW_SLUG}"  (passport "${erik.passport?.displayName}", node erik-paulson)`)
  }
  ...
  const keeperOnCanonicalPub = keeper.treeMembers.some(m => m.tree.slug === "rigan-machado-lineage" && m.tree.isPublished)
  if (!keeperOnCanonicalPub) throw new Error(`GUARD: keeper ${RENATO_KEEP_NODE_SLUG} not on published canonical — aborting`)

  const dup = await db.lineageNode.findUnique({
    where: { id: RENATO_DUP_NODE_ID },
    select: {
      id: true, slug: true, passportId: true,
      passport: { select: { id: true, displayName: true, userId: true, rankAwardsEarned: { select: { id: true } }, affiliations: { select: { id: true } }, directoryProfile: { select: { id: true, slug: true } } } },
  ...
  if (!dup) {
    console.log(`FIX 2 (Renato dup): SKIP — node ${RENATO_DUP_NODE_ID} not found (already removed?)`)
  } else {
    if (dup.passportId !== RENATO_DUP_PASSPORT_ID) throw new Error(`GUARD: dup passportId ${dup.passportId} ≠ expected — aborting`)
    if (dup.passport?.userId) throw new Error(`GUARD: dup passport has a User (${dup.passport.userId}) — refusing to delete a claimed identity`)
    ...
    if (anchoredEdges.length > 0) throw new Error(`GUARD: dup anchors ${anchoredEdges.length} student edge(s) — would orphan; aborting`)
    const dupMemberIds = dup.treeMembers.map(m => m.id)
    const visualChildren = dupMemberIds.length
      ? await db.lineageTreeMember.count({ where: { primaryVisualParentMemberId: { in: dupMemberIds } } })
      : 0
    if (visualChildren > 0) throw new Error(`GUARD: ${visualChildren} member(s) visual-parent to the dup — would fall to root; aborting`)
    if (dup.passport && dup.passport.rankAwardsEarned.length > 0) throw new Error(`GUARD: dup passport has ${dup.passport.rankAwardsEarned.length} rank award(s) — not a bare duplicate; review before delete`)

    console.log(`\nFIX 2 (Renato dup): DELETE duplicate "${dup.slug}" (passport "${dup.passport?.displayName}")`)
    console.log(`  keeper: ${keeper.slug} — ${keeper.passport?.rankAwardsEarned.length} award(s), ${keeper.relationshipsFrom.length} student edge(s), on published canonical ✓`)
    console.log(`  cascade on Passport delete → node ${dup.id}, ${dup.treeMembers.length} member(s), ${dup.relationshipsFrom.length + dup.relationshipsTo.length} edge(s), directoryProfile ${dup.passport?.directoryProfile?.slug ?? "none"}`)
    console.log(`  dup passport: 0 User, ${dup.passport?.rankAwardsEarned.length} awards, ${dup.passport?.affiliations.length} affiliations — bare duplicate ✓`)
  }
  ...
  await db.$transaction(async tx => {
    if (erik?.slug === ERIK_OLD_SLUG) {
      await tx.directoryProfile.update({ where: { id: ERIK_PROFILE_ID }, data: { slug: ERIK_NEW_SLUG } })
    }
    if (dup) {
  ...
  console.log("\n✅ APPLIED.")
  console.log(`   Erik slug: ${erik?.slug === ERIK_OLD_SLUG ? `renamed → ${ERIK_NEW_SLUG}` : "skipped"}`)
