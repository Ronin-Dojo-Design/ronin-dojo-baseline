...
async function main() {
  ...
  for (const recipient of queue) {
    try {
      ...
      const claimUrl = `${opts.baseUrl}/lineage/join?node=${resolved.nodeId}`

      if (opts.dryRun) {
        console.log(
          `  ✓ would send → ${recipient.email} (${resolved.profileName}, ${recipient.dirtyDozen ? "lifetime" : "1yr"}) ${claimUrl}`,
        )
        sent += 1
        continue
      }
