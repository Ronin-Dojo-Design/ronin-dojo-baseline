/**
 * SESSION_0500 (Epic B) — mobile-viewport proof for the bottom nav + radial MAB.
 *
 * Verifies at a 390px mobile viewport:
 *  - Bottom nav renders for a signed-in user (5 tabs) with active-route highlight.
 *  - ADMIN sees the movable radial MAB; a NON-ADMIN does NOT.
 *  - MAB fan opens; drag repositions + snaps to a corner; the position + toggle-off
 *    persist across reload (localStorage).
 *  - The Upload action opens the bottom-sheet.
...
test.describe("Epic B mobile shell", () => {
  test("admin: bottom nav + MAB drag/persist/toggle + bottom-sheet", async ({ page }) => {
    ...
    try {
      ...
      await page.screenshot({ path: `${SHOTS}/admin-02-fan-open.png` })
      ...
      await page.screenshot({ path: `${SHOTS}/admin-03-upload-sheet.png` })
      ...
      await page.screenshot({ path: `${SHOTS}/admin-04-dragged-topleft-persist.png` })
      ...
      await page.reload()
      await expect(page.locator(MAB_TRIGGER)).toHaveCount(0)
      await page.screenshot({ path: `${SHOTS}/admin-05-toggled-off.png` })
      ...
      await page.screenshot({ path: `${SHOTS}/admin-06-desktop-hidden.png` })
    } finally {
      await cleanupTestUser(admin.userId)
    }
  })

  test("non-admin: bottom nav but NO MAB", async ({ page }) => {
    page.setViewportSize(MOBILE)
    ...
    try {
      ...
      const nav = page.getByRole("navigation", { name: /browse/i }).last()
      await expect(nav).toBeVisible()
      // Signed-in member gets the full 5-tab set (Dashboard/Profile are session-gated —
      // wait for useSession to hydrate them, proving the subset in the guest SSR is timing,
      // not a bug). Active Directory tab is highlighted.
