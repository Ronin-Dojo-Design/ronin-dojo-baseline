...
export function AwardCard({ award }: { award: AwardRow }) {
  ...
  const promoteeName = award.passport.displayName ?? award.passport.user?.name ?? "Unnamed promotee"
  const promoteeAvatar = award.passport.avatarUrl ?? award.passport.user?.image ?? undefined
