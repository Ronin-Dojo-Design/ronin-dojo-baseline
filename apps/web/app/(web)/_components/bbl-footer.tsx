...
/**
 * Black Belt Legacy footer for the pre-launch holding page — a brand column
 * (logo + tagline + social), a contact bar, and a copyright bar. The legacy
 * link columns (Lineage Network / Resources / Legal) are intentionally omitted
 * for now while the destinations are still gated.
 */
...
// Pre-launch: BBL social accounts aren't live yet, so the holding-page footer
// hides them for the moment. Flip to true to surface them post-launch.
...
export function BblFooter() {
  ...
  return (
    <footer
      className={`${bblHeadingFont.variable} ${bblBodyFont.variable} border-t border-white/10 bg-[#0a0a0a] text-white/60 [font-family:var(--font-bbl-body),system-ui,sans-serif]`}
    >
      <div className="mx-auto max-w-6xl px-6 py-12 sm:px-10">
        ...
        <p className="mt-4 max-w-md text-sm/6 text-white/50">
          Building the definitive lineage network for martial artists — track your journey, verify
          your credentials, and connect with instructors from Rigan Machado&apos;s legendary black
          belt lineage.
        </p>
        {/* Social links hidden for the pre-launch holding page (accounts not live yet);
            flip SHOW_SOCIAL to re-enable post-launch. */}
        {SHOW_SOCIAL && (
