 ...
 *
 * Both arms share the reused `ListingDetail` chrome + `BjjPassportCard` credential; the shared
 * SECTION concepts (hero badges/actions, about, sidebar, ranks/belt-history) each have one
 * implementation per arm reconciled through the viewer context — no third copy. The tier
 * contract (`canRenderProfile` / `canRenderRichMedia`) is applied entirely in the projection;
 * the public arm consumes the already-gated fields, so rendering stays presentation-only.
 */
