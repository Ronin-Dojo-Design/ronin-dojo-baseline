...
export const getColumns = (): ColumnDef<AgeGroup & { _count?: { programs: number } }>[] => {
  return [
    selectColumn<AgeGroup & { _count?: { programs: number } }>(),
    {
      accessorKey: "name",
      enableHiding: false,
      size: 160,
      ...
      cell: ({ row }) => (
        <DataTableLink href={`/app/age-groups/${row.original.id}`} title={row.original.name} />
      ),
    },
    {
      accessorKey: "code",
