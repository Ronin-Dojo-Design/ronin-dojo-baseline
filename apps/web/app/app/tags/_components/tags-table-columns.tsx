...
import { TagActions } from "~/app/app/tags/_components/tag-actions"
...
export const getColumns = (): ColumnDef<Tag & { _count?: { tools: number } }>[] => {
  return [
    selectColumn<Tag & { _count?: { tools: number } }>(),
    {
      accessorKey: "name",
      enableHiding: false,
      size: 160,
      ...
      cell: ({ row }) => (
        <DataTableLink href={`/app/tags/${row.original.slug}`} title={row.original.name} />
      ),
    },
    {
