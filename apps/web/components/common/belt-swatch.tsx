 ...
 * brand-safe rule, ADR 0022 / lineage surfaces). The belt BODY color always comes
 * from `Rank.colorHex`; only the rank-bar TREATMENT (bar color + outline) is design,
 * driven by the belt FAMILY — see `beltBarTreatment` below, the ONE home for those
 * design constants.
 *
 ...
 * text carries the meaning. Falls back to a neutral muted swatch (`currentColor` +
 * `text-muted`) when a rank has no `colorHex`.
 *
 * Variants:
...
// colours still come from `Rank.colorHex` (ADR 0022 intact).
  ...
  if (variant === "belt") {
    ...
    return (
      <svg
        ...
        className={cx("h-auto shrink-0", SIZE_WIDTH[size], !colorHex && "text-muted", className)}
      >
