...
import type { BblGalaxyEdge, BblGalaxyGraph, BblGalaxyGroup, BblGalaxyNode, BblGalaxyRole } from "./bbl-galaxy-types"
...
const rankLabelOf = (member: MemberRow): string | undefined => {
  const award = member.selectedRankAward ?? member.node.passport?.rankAwardsEarned?.[0]
  const rankName = award?.rank?.name
  const discipline = award?.rank?.rankSystem?.discipline?.name
  if (!rankName) return undefined
  return discipline ? `${rankName} · ${discipline}` : rankName
}
...
export const lineageTreeToGalaxyGraph = (result: LineageTreePublicResult): BblGalaxyGraph => {
  ...
  const nodes: BblGalaxyNode[] = members.map(member => {
    ...
    const displayName = passport?.displayName ?? passport?.user?.name ?? "Unknown lineage holder"
    ...
    return {
      ...
      discipline: "BJJ",
      rankLabel: rankLabelOf(member),
      photoUrl: passport?.avatarUrl ?? passport?.user?.image ?? undefined,
