...
import type { BblGalaxyEdge, BblGalaxyGraph, BblGalaxyGroup, BblGalaxyNode } from "./bbl-galaxy-types"
import { createBblGalaxyLayout, getGalaxyNodeSize, mergePositionsIntoNodes } from "./bbl-galaxy-layout"
...
function GalaxyShell({
  ...
  const nodes = useMemo(() => mergePositionsIntoNodes(graph.nodes, positions), [graph.nodes, positions])
  ...
  return (
    <>
      ...
      {nodes.map((node) => (
        <GalaxyNode key={node.id} node={node} isSelected={selectedNode?.id === node.id} onSelectNode={onSelectNode} />
      ))}
...
function GalaxyNode({
  ...
  return (
    <group ref={groupRef} position={[node.position.x, node.position.y, node.position.z]}>
      ...
      <Html center distanceFactor={node.role === "ROOT_STAR" ? 8 : 10} position={[0, size + 0.35, 0]}>
        <button
          type="button"
        ...
        >
          <span className="flex items-center gap-2">
            ...
            <span className="min-w-0">
              <span className="block truncate text-[11px] font-semibold leading-tight">{node.displayName}</span>
              {node.rankLabel ? (
                <span className="block truncate text-[9px] uppercase tracking-[0.18em] text-white/60">{node.rankLabel}</span>
              ) : null}
            </span>
          </span>
        </button>
      ...
      {edges.map((edge) => {
        ...
        return (
          <line key={edge.id} geometry={geometry}>
            <lineBasicMaterial color={isPrimary ? "#d7a74c" : "#7fd7ff"} transparent opacity={isPrimary ? 0.42 : 0.24} />
          </line>
        )
      })}
    </>
...
function CameraZoomController({ selectedNode, nodes }: { selectedNode: BblGalaxyNode | null; nodes: PositionedNode[] }) {
  ...
  useEffect(() => {
    ...
    const target = new THREE.Vector3(positioned.position.x, positioned.position.y, positioned.position.z)
    const direction = target.clone().normalize()
    const safeDirection = direction.length() > 0 ? direction : new THREE.Vector3(0, 0.25, 1)
    const cameraTarget = target.clone().add(safeDirection.multiplyScalar(3.8)).add(new THREE.Vector3(0, 1.25, 2.5))
    ...
    <div className="pointer-events-none absolute inset-0 z-10 flex flex-col justify-between p-5">
      <div className="flex items-start justify-between gap-4">
        <div className="max-w-xl rounded-2xl border border-white/10 bg-black/55 p-4 text-white shadow-2xl backdrop-blur-md">
          <p className="text-xs uppercase tracking-[0.35em] text-yellow-200/80">Black Belt Legacy</p>
          <h2 className="mt-1 text-2xl font-semibold tracking-tight">Lineage Galaxy</h2>
          <p className="mt-2 text-sm leading-6 text-white/70">
            A cinematic map of verified public lineage. Stars mark legends, planets mark instructors, and moons mark students.
          </p>
        </div>
      ...
      <div className="flex items-end justify-between gap-4">
        <div className="rounded-2xl border border-white/10 bg-black/55 p-4 text-xs text-white/70 backdrop-blur-md">
          <div className="grid gap-2">
            <div><span className="text-yellow-200">Gold stars</span> - lineage anchors</div>
            <div><span className="text-cyan-200">Blue planets</span> - instructors</div>
            <div><span className="text-white">White moons</span> - students</div>
          </div>
        </div>
...
export function BblLineageGalaxy({ graph, onSelectNode }: BblLineageGalaxyProps) {
  ...
  return (
    <section className="relative h-[760px] w-full overflow-hidden rounded-[2rem] border border-white/10 bg-[#02030a] shadow-2xl">
      <GalaxyHud selectedNode={selectedNode} onReset={handleReset} onFollowPath={handleFollowPath} />
