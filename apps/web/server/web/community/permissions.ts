 ...
 * behavior change; grandfather is forward-only, existing posts stay published). Mirrors
 * `canCreateTechniqueForUser` exactly — it composes the SAME three existing seams (NOT a 5th authz
 * system):
 ...
 * reference-stable within a request). The CHEAP/common entitlement leg runs BEFORE the uncached
 * membership query, so the hot path (a paid member with no staff role) skips the second DB roundtrip
 * — the same ordering rationale as the technique gate.
 */
