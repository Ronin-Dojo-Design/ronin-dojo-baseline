 ...
 */
async function fulfillTournamentRegistration(
  session: Stripe.Checkout.Session,
  stripeClient: Stripe,
) {
  ...
  try {
    await stripeClient.refunds.create({ payment_intent: result.paymentIntentId })
  } catch (error) {
...
/**
 * Process a Stripe webhook request against a specific Stripe account.
 *
 * The account is fixed by the **endpoint that received the event** (and thus the
 * verifying secret), not by the brand in the payload — `listLineItems` runs
 * before the brand is known, so it must use the same account the event came
 * from. Each brand's route passes its own `(stripeClient, webhookSecret)` pair:
 * the shared platform account for Baseline/Ronin/WEKAF, the dedicated BBL
 * account for Black Belt Legacy.
 *
 * @param req - The request
 * @param stripeClient - The Stripe client for the receiving account
 * @param webhookSecret - The signing secret for the receiving account
