...
/**
 * Sibling worktrees' `docs/` trees (WL-P3-66, SESSION_0624).
 *
 * Until now ONLY the `SESSION` mint looked across worktrees; every other prefix scanned this
 * checkout alone. That is a live collision generator, not a theoretical one: SESSION_0598 nearly
 * minted `G-026` for the RDD deploy while a sibling lane had already committed a DIFFERENT `G-026`
 * on its own branch — caught only because a human happened to read the goals ledger first.
 * Parallel lanes are the norm now, so "the current tree" is the wrong denominator for "is this
 * number free".
 *
 * KNOWN LIMIT, and it bit SESSION_0624: the scan counts every occurrence in `docs/`, including
 * PROSE and including your own UNCOMMITTED draft. Write "`WL-P3-64`" into a doc, then mint, and
 * the mint reports 64 as taken — by you, seconds ago. Draft the row with the number the mint
 * gives you; do not mint after writing the id. (0624 burned WL-P3-64 exactly this way. Gaps stay
 * burned per ADR 0049, so it stays burned.)
 *
 * Read-only, best-effort: an unreadable worktree is skipped rather than fatal, so a stale
 * `git worktree list` entry can never block a mint. Branch refs are NOT scanned here — unlike
 * `session-NNNN-*` branch names, a `G-`/`WL-` id never appears in a branch name, so the docs
 * trees of mounted worktrees are the only cross-lane signal available.
 */
function siblingWorktreeDocDirs(): string[] {
  try {
    return execSync("git worktree list --porcelain", { cwd: ROOT, encoding: "utf-8" })
      .split("\n")
      .filter(l => l.startsWith("worktree "))
      .map(l => l.slice("worktree ".length))
      .filter(wt => resolve(wt) !== ROOT)
      .map(wt => join(wt, "docs"))
      .filter(d => existsSync(d))
  } catch {
    return [] // not a git checkout, or git unavailable — the local scan stands alone
  }
}

/**
 * `isLocal` distinguishes THIS checkout's docs from a sibling worktree's copy, and the two
 * consumers want opposite things:
 *   - the MINT (`usedNumbers`) wants every tree — a number claimed in a sibling is claimed;
 *   - the DUP CHECK wants local only — `drift-register.md:32` existing in four worktrees is ONE
 *     definition seen four times, not four collisions. Scanning siblings there reported 317
 *     phantom "duplicates" (caught by running `--check` after the change, SESSION_0624).
 */
const FILES = [DOCS, ...siblingWorktreeDocDirs()].flatMap(dir => {
  const isLocal = dir === DOCS
  let paths: string[] = []
  try {
    paths = walkMarkdown(dir)
  } catch {
    return [] // unreadable worktree — skip, never fail the mint
  }
  return paths.flatMap(p => {
    try {
      return [{ path: p, rel: relative(ROOT, p), text: readFileSync(p, "utf-8"), isLocal }]
    } catch {
      return []
    }
  })
...
const LEDGER_FILE: Partial<Record<string, string>> = {
  ...
  DES: "knowledge/wiki/desi-design-ledger.md",
...
function definitionSites(prefix: string, n: number, text: string, rel: string): string[] {
  ...
  // A resolved row writes its status glyph INTO the id cell — `| WL-P3-66 ✅ |` — because
  // `lib/loop-board/ledger-parse.ts` only counts a ✅ there or at the head of the status cell
  // (SESSION_0624: WL-P3-54 read as OPEN to the board for exactly this reason). Without
  // tolerating the glyph here, the two parsers disagree: the board sees a resolved row while
  // `--check` sees no definition at all and reports the id as a phantom. Allow a short run of
  // trailing status glyphs, but nothing else — a real row is still `| <id>[ glyph] |`.
  const rowRe = new RegExp(`^\\|\\s*\`?\\*{0,2}${escaped}\\*{0,2}\`?\\s*[✅❌✔️☑️~]{0,2}\\s*\\|`)
  text.split("\n").forEach((line, i) => {
...
if (CHECK) {
  // Local only — a sibling worktree's copy of the same ledger is the same row, not a collision.
  const liveFiles = FILES.filter(f => f.isLocal && !HISTORY_PREFIXES.some(p => f.rel.startsWith(p)))
