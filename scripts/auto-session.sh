...
0 errors), run `bun run typecheck`, and run the read-only Oxc gates
`(cd apps/web && bun run lint:check && bun run format:check)`. DO NOT run the root
`bun run lint` or the apps/web `lint`/`format` scripts — those use `--fix`/write and
MUTATE files (the old FS-0017 `biome` PATH gap is moot post-Oxc migration, SESSION_0360);
the real gate is "oxlint + oxfmt check + typecheck". Then write the hostile close review and run
