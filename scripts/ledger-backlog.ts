 ...
 * DBS · SSL. `PR` is a LIVE source — queried from `gh pr list` (not a file) and parsed by the SAME
 ...
 *   bun scripts/ledger-backlog.ts --ledger=GL     # one source (GL|PR|FS|D|WL|FI|MB|TFF|INC|RISK|TD|PL|RLL|YLL|GPTLL|DBS|SSL)
...
const LEDGER_FILTER = (ARGS.find((a) => a.startsWith("--ledger=")) ?? "")
  .split("=")[1]
  ?.toUpperCase() as LedgerCode | undefined;
...
function fetchPullRequestItems() {
  ...
  try {
    ...
    if (!JSON_OUT) {
      console.error(
        "  (PR source skipped — `gh` unavailable, unauthenticated, or no repo on this cwd)",
      );
    }
    return [];
  }
...
const counts = LEDGER_ORDER.map(
  (code) => `${code} ${items.filter((i) => i.ledger === code).length}`,
);
...
console.log(
  `  ${"-".repeat(4)}${"-".repeat(14)}${"-".repeat(8)}${"-".repeat(18)}${"-".repeat(40)}`,
);
