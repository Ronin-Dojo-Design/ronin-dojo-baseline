...
:root{--code:var(--surface);--sidebar:var(--surface)}
...
.brand{font-weight:700;letter-spacing:-.02em;font-size:15px;margin:2px 6px 4px}
...
.search{width:100%;margin:14px 0 18px;padding:8px 11px;border:1px solid var(--line);border-radius:9px;background:#fff;font-size:13px}
...
.count,.badge{font-weight:600;color:var(--muted);background:#ececef;border-radius:20px;font-size:10px;padding:1px 7px}
.leaf{display:flex;justify-content:space-between;gap:8px;align-items:center;padding:6px 8px;border-radius:7px;color:#2b2b30;font-size:13px}
.leaf:hover{background:#f0f0f3}
.leaf.active{background:#fff;box-shadow:inset 2px 0 0 var(--accent);font-weight:600}
...
.doc blockquote{margin:1em 0;padding:2px 16px;border-left:3px solid var(--accent);color:var(--muted);background:#fcfbf7}
...
.doc ol>li::before{content:counter(s);position:absolute;left:0;top:2px;width:24px;height:24px;border-radius:50%;background:var(--accent);color:#fff;font-size:12px;font-weight:700;display:grid;place-items:center}
...
<aside>
  <div class="brand">Orchestration Hub<small>Ronin Dojo · agent handoff</small></div>
