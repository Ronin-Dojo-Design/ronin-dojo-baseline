...
`bun run wiki:lint` (it MUST report 0 errors), run typecheck/lint, write the hostile
close review, and run `graphify update` BEFORE the commit (FS-0025 single-push order).
